...
public class FloorAction extends org.apache.struts.action.Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form,
        ...
        busyTables = manager.findAllBusyTable(floors.get(0).getId());
        Iterator iter = busyTables.iterator();
        int i = 0;
        Place pl = null;
        TicketInfo tk = null;
        while (iter.hasNext()){
          pl = (Place) busyTables.get(i);
          tk = manager.findTicket(pl.getId());
          if (tk.getLinesCount()<=0)
              manager.deleteTicket(pl.getId());

          i = i + 1;
          if (i >= busyTables.size())
             break;
        }
